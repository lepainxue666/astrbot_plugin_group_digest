...
class ChatSummary(Star):
    ...
    def __init__(self, context: Context, config: dict | None = None):
        ...
        # 记录免打扰模式下上次回复的用户和时间，防止重复回复
        self._dnd_last_reply: Dict[str, datetime] = {}
    ...
    async def handle_private_message(self, event: AstrMessageEvent):
        ...
        logger.info("免打扰模式：收到私聊消息，开始处理")
        
        # 检查是否在免打扰时间段内
        is_dnd = self._is_dnd_time()
        logger.info("免打扰模式：当前是否在免打扰时间段: %s", is_dnd)
        if not is_dnd:
            logger.info("免打扰模式：不在免打扰时间段，跳过处理")
            return
        
        # 检查用户是否在白名单中
        sender_id = event.get_sender_id()
        in_whitelist = self._is_in_whitelist(sender_id)
        logger.info("免打扰模式：用户 %s 是否在白名单: %s", sender_id, in_whitelist)
        if in_whitelist:
            logger.info("免打扰模式：用户在白名单，跳过处理")
            return
        
        # 检查是否为实际消息（过滤状态消息、空消息等）
        message_text = self._extract_message_text(event)
        logger.info("免打扰模式：提取到的消息文本: '%s'", message_text)
        if not message_text or len(message_text.strip()) == 0:
            logger.info("免打扰模式：消息为空，跳过处理")
            return
        
        # 检查是否在短时间内重复回复（防止对方正在输入时多次触发）
        sender_key = str(sender_id)
        now = datetime.now()
        if sender_key in self._dnd_last_reply:
            last_reply_time = self._dnd_last_reply[sender_key]
            time_diff = (now - last_reply_time).total_seconds()
            logger.info("免打扰模式：距离上次回复 %s 秒", time_diff)
            # 如果距离上次回复不到 60 秒，不再回复
            if time_diff < 60:
                logger.info("免打扰模式：距离上次回复不足60秒，跳过处理")
                return
        
        # 获取自动回复消息
        auto_reply = self._get_dnd_auto_reply()
        logger.info("免打扰模式：准备发送自动回复: '%s'", auto_reply)
    ...
    async def handle_group_message(self, event: AstrMessageEvent):
        ...
        # 检查是否@了机器人
        if not self._is_mentioned(event, bot_id):
            return
        
        # 检查是否在短时间内重复回复
        group_key = f"{group_id}_{sender_id}"
        now = datetime.now()
        if group_key in self._dnd_last_reply:
            last_reply_time = self._dnd_last_reply[group_key]
            # 如果距离上次回复不到 60 秒，不再回复
            if (now - last_reply_time).total_seconds() < 60:
                return
        ...
        # 记录回复时间
        self._dnd_last_reply[group_key] = now
        
        logger.info("免打扰模式：群聊 %s 中已自动回复用户 %s 的@消息", group_id, sender_id)
    ...
    def _extract_message_text(self, event: AstrMessageEvent) -> str:
        ...
        try:
            # 直接从 event 对象获取消息内容
            message_attr = getattr(event, 'message', None)
            if message_attr and isinstance(message_attr, list):
                # 提取文本内容
                text_parts = []
                for part in message_attr:
                ...
                logger.debug("免打扰模式：提取到的消息文本: '%s'", result)
                return result
            ...
            logger.warning("免打扰模式：无法获取消息内容")
            return ""
        except Exception as exc:
            logger.warning("提取消息文本失败: %s", exc)
            return ""

    def _is_mentioned(self, event: AstrMessageEvent, bot_id: str | int) -> bool:
        """检查消息是否@了机器人"""
        try:
            # 直接从 event 对象获取消息内容
            message_attr = getattr(event, 'message', None)
